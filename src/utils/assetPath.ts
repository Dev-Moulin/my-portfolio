...
/**
 * Raccourci pour les modèles 3D dans le dossier public/Overmind
 * @param modelName - Le nom du fichier de modèle 3D
 * @returns Le chemin complet vers le modèle 3D
 */
export const getModelPath = (modelName: string): string => {
  return getAssetPath(`Overmind/${modelName}`);
};